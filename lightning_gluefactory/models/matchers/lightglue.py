...
class LightGlue(BaseModel):
    ...
    def _forward(self, data: dict) -> dict:
        ...
        do_early_stop = self.depth_confidence > 0 and not self.training
        do_point_pruning = self.width_confidence > 0 and not self.training
    ...
    def pruning_min_kpts(self, device: torch.device):
        if self.flash and FLASH_AVAILABLE and device.type == "cuda":
